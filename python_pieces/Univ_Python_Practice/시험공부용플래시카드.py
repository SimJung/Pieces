...
# 키 입력 함수:
def click():
    entered_text = entry.get(0.0, END)
    print(entered_text)
...
def getq():
    ...
    question = questions[random.randint(0,questions.__len__()-1)]
    entry.insert(0.0, question)

##### 메인:
window = Tk()
window.title("My Coding Club Glossary")

button1 = Button(window, width=10, text="질문 얻기", bg="White", command=getq)
...
entry = Text(window, width=20, height=1, wrap=WORD, bg="light green")
